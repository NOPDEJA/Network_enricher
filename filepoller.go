...
// source is used only for log context. It never recovers panics — the caller's
// scan() owns the recover so a bad line is counted, not silently fatal.
func scanAppendedDir(dir, source string, offsets map[string]int64, handle func(line string)) {
	if dir == "" {
		return
	}
...
func readAppendedFile(path, source string, offsets map[string]int64, handle func(line string)) {
	fi, err := os.Stat(path)
	if err != nil {
		return
	}
